...
QPushButton* KColorBox::getColorButton() const
{
	return m_pColorBtn;
}

void KColorBox::shapeColorChanged()
{
	QPalette palette = m_pColorBtn->palette();

	// 获取按钮背景颜色
	QColor color = palette.color(QPalette::Button);

	QString colorString = color.name();
	m_pColorBtn->setStyleSheet(QString("background-color: %1;").arg(colorString));
	//qDebug() << colorString;

	emit shapeColor(colorString);
	
}

void KColorBox::lineColorChanged()
{
	QPalette palette = m_pColorBtn->palette();
