...
*/
	...
	//QRegExp reg("[1-9]+(\\.[0-9]+)?");//修改这个正则表达式，这样可以识别小数
	QRegExp reg("^[1-9]\\d*(\\.[0-9]+)?$");
...
void KValueBox::setScaleValue()
{

	m_pParamEdit->setText(QString::number(KGlobalData::getGlobalDataIntance()->getCanvasScale()));

}

void KValueBox::setWidthScaleValue()
{
	m_pParamEdit->setText(QString::number(KGlobalData::getGlobalDataIntance()->getCanvasWidth() * KGlobalData::getGlobalDataIntance()->getCanvasScale()));
}

void KValueBox::setHeightScaleValue()
{
	m_pParamEdit->setText(QString::number(KGlobalData::getGlobalDataIntance()->getCanvasHeight()* KGlobalData::getGlobalDataIntance()->getCanvasScale()));
}
